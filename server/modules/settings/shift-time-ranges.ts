/** Pure time-range helpers — ShiftType is the source of truth for working hours. */
...
export function findShiftTypeOverlapError(
  ...
  others: Array<ShiftTypeTimeCandidate & { isActive?: boolean }>,
): string | null {
...
export function assertValidShiftTypeTimes(startTime: string, endTime: string): void {
  const err = validateShiftTypeTimeRange(startTime, endTime);
  if (err) {
