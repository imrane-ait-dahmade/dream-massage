...
import { Plus, UserX, UserCheck, UserPlus } from 'lucide-react';
    ...
    <div className="space-y-2.5 rounded-xl border border-stone-200 bg-stone-50 p-3">
      ...
      <div className="flex flex-col gap-0.5">
        ...
        <textarea
          ...
          rows={2}
          placeholder="optionnel"
          className="resize-none rounded-lg border border-stone-200 bg-white px-2.5 py-1.5 text-sm outline-none focus:border-stone-400"
        />
      </div>
  ...
  return (
    <div
      className={`flex items-center justify-between gap-2 rounded-xl border px-3 py-2.5 ${
        member.isActive ? 'border-stone-100 bg-white' : 'border-stone-100 bg-stone-50'
      }`}
    >
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <p
            className={`truncate text-sm font-medium ${
              member.isActive ? 'text-stone-900' : 'text-stone-400'
            }`}
          >
            {member.name}
          </p>
        ...
        {member.notes && (
          <p className="truncate text-xs text-stone-400">{member.notes}</p>
        )}
      </div>
      <div className="flex shrink-0 items-center gap-2">
        ...
        <button
          ...
          className="text-xs text-stone-400 underline-offset-2 hover:text-stone-700"
        >
          Modifier
        </button>
      </div>
...
export function StaffSettings({
  ...
  async function handleCreate(form: StaffForm) {
    ...
    setCreateFeedback({ type: 'success', msg: 'Assistante ajoutée' });
    setCreating(false);
    onSaved();
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5 text-xs text-stone-500">
        Les assistantes n&apos;ont pas de compte et ne se connectent pas à l&apos;application.
        Elles sont uniquement utilisées pour le planning et les shifts.
      </div>
      ...
      <div className="space-y-2">
        {active.length === 0 && inactive.length === 0 && (
          <p className="py-6 text-center text-sm text-stone-400">
            Aucune assistante ajoutée.
          </p>
        )}
        {active.map((m) => (
          <StaffRow key={m.id} member={m} onSaved={onSaved} />
        ))}
        {inactive.length > 0 && (
          <>
      ...
      {creating ? (
        <StaffFormPanel
          submitLabel="Ajouter l'assistante"
          onSave={handleCreate}
          onCancel={() => { setCreating(false); setCreateFeedback(null); }}
        />
      ) : (
        <button
          onClick={() => { setCreating(true); setCreateFeedback(null); }}
